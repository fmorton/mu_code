            ...
            async with BleakClient(device) as client:
                if client.is_connected:
                    ...
                    for service in services:
                        ...
                        if (
                            "180A" in service.uuid
                        ):  # Check for Device Information Service
                            for char in service.characteristics:
                                print(f"    Characteristic: {char.uuid}")
                                if (
                                    "2A29" in char.uuid
                                ):  # Check for Manufacturer Name String
                                    manufacturer_bytes = await client.read_gatt_char(
                                        char.uuid
                                    )
...
device_address = (
    "DE22C91F-84A3-944D-8653-021AB96CB12A"  # Replace with the actual device address
)
